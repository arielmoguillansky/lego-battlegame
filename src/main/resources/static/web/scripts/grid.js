...
var opponent;
  for(let i = 0; i < gamesData.gamePlayers.length; i++){
    if(gamesData.gamePlayers[i].gpid == gpId){
      actualPlayer = gamesData.gamePlayers[i].gpid;
    }else{opponent = gamesData.gamePlayers[i].gpid}
    }

    for(i = 0; i < gamesData.salvoes.length; i++){
        for(j = 0; j < gamesData.salvoes[i].locations.length; j++){
            let turn = gamesData.salvoes[i].turn
            let player = gamesData.salvoes[i].game_player_id
            ...
            if(player == actualPlayer.id){
                document.getElementById(`salvoes${y}${x}`).classList.add('smash')
            }else{
...
var opponent;
  for(let i = 0; i < gamesData.gamePlayers.length; i++){
    ...
    for(i = 0; i < gamesData.salvoes.length; i++){
        for(j = 0; j < gamesData.salvoes[i].hits.length; j++){
            let turn = gamesData.salvoes[i].turn
            let player = gamesData.salvoes[i].game_player_id
            let x = +(gamesData.salvoes[i].hits[j][1]) - 1
            let y = stringToInt(gamesData.salvoes[i].hits[j][0].toUpperCase())

            if(player == actualPlayer){
                document.getElementById(`salvoes${y}${x}`).classList.remove('smash');
